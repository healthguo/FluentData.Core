...
namespace FluentData.Test.IntegrationTests.Models
{
	public static class TestHelper
	{
		...
		{
			/*var appSettings = XDocument.Load(@"d:\OneDrive\AppSettings\FluentData.FluentData.Test.IntegrationTests\App.config");
			...
			return "Data Source=|DataDirectory|\\UnitTests\\test.db";

        }

		public static Product GetProduct(IDbContext context, int productId)
		{
			var product = context.Sql("select * from product where productid = @0", productId)
							.QuerySingle<Product>();
			return product;
		}
