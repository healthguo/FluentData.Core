...
{
    /// <summary>
    /// Generates UPDATE SQL statements from builder data.
    /// </summary>
    internal class UpdateBuilderSqlGenerator
    {
        ...
        /// <param name="data">The builder data containing columns, WHERE clause, and table name.</param>
        /// <returns>A formatted UPDATE SQL statement with SET and WHERE clauses.</returns>
        public string GenerateSql(IDbProvider provider, string parameterPrefix, BuilderData data)
        {
            ...
            return string.Format("update {0} set {1} where {2}", data.ObjectName, setSql, whereSql);
        }
    }
}
